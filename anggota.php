...
<table class="table table-striped table-hover">
    ...
    <?php
        ...
        // Query Join 3 tabel
        $querybaru = mysqli_query($konek, "SELECT anggota.nis, anggota.nama, anggota.jk, anggota.tpt_lahir, 
        anggota.tgl_lahir, anggota.id_kelas, anggota.id_jurusan, anggota.tlp, anggota.alamat, kelas.nama_kelas, 
        jurusan.nama_jurusan
        FROM anggota
        JOIN kelas ON anggota.id_kelas = kelas.id_kelas
        JOIN jurusan ON anggota.id_jurusan = jurusan.id_jurusan");
        // End query Join tabel

        // $query = mysqli_query($konek,"SELECT * FROM anggota");
        $no = 1;
        foreach ($querybaru as $row) {
    ?>
    <tr align="center" style="font-size: 11px; table-layout: fixed">
